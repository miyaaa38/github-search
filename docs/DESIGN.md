...
ライブラリ・OSS 調査を行うエンジニアを対象としています。典型的な行動は「キーワードで検索 → Star や更新状況で比較 → 詳細で採否を判断 → 必要なら URL で共有」という一連の流れで、これを 5〜10 秒で完了できることを設計目標にしました。

この行動モデルから導かれる設計上の要請は以下の 3 点です：

- **初回表示の速さ** — 検索 → 比較までの時間が体験の中核
- **URL で状態を再現できること** — ブラウザバック・タブ復元・第三者への共有が破綻しない
- **並び替えが URL に乗ること** — 「Stars 順で見ていた結果を共有する」ユースケースを成立させる
...
| `useState`                       | 実装が最も簡単                                                                | URL に状態が乗らず、共有・復元ができない。想定ユーザーの「検索結果を他者と共有する」行動が破綻             |
